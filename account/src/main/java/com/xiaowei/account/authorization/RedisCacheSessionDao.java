...
public class RedisCacheSessionDao extends CachingSessionDAO {
    ...
    protected Serializable doCreate(Session session) {
        ...
        redisTemplate.opsForValue().set(AccountConst.USER_REDIS_GROUP_PREFIX +id, session,sessionTimeOut, TimeUnit.MINUTES);
    ...
    protected void doUpdate(Session session) {
        redisTemplate.opsForValue().set(AccountConst.USER_REDIS_GROUP_PREFIX + session.getId(), session, sessionTimeOut, TimeUnit.MINUTES);
    }

    @Override
    protected void doDelete(Session session) {
        redisTemplate.opsForValue().getOperations().delete(AccountConst.USER_REDIS_GROUP_PREFIX +session.getId());

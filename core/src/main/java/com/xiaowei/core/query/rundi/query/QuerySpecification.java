...
import com.xiaowei.core.query.yuanxuan.jpa.PredicateBuilder;
import com.xiaowei.core.query.yuanxuan.jpa.Sorts;
import com.xiaowei.core.query.yuanxuan.jpa.Specifications;
...
public class QuerySpecification<T>{
    ...
    public void toPredicate(List<Filter> filters, PredicateBuilder<T> predicate){
        for (Filter filter : filters) {
            ...
            switch (filter.getOperator()){
                ...
                case eq:
                    if(!StringUtils.isEmpty(filter.getOneValue())){
                        predicate.eq(filter.getField(),filter.getOneValue());
                    }
                    break;

                case ge:
                    try {
                        Long value = new Long(filter.getOneValue().toString());
                ...
                case in:
                    if(!ArrayUtils.isEmpty(filter.getValue())){
                ...
                case like:
                    if(!StringUtils.isEmpty(filter.getOneValue())){
                        predicate.like(filter.getField(),filter.getOneValue().toString());
                    }
                    break;
                ...
                case neq:
                    if(!StringUtils.isEmpty(filter.getOneValue())){
                        predicate.ne(filter.getField(),filter.getOneValue());
                    }
                    break;
    ...
    public static Object[] guess(Object ...values) {
        ...
        for (int i = 0; i < values.length; i++) {
            ...
            try {
                if (values[i] instanceof Date) {//如果是时间就不管了
                    guessValues[i] = values[i];
                } else {
    ...
    public Specification<T> buildSpecification(Query query){
        ...
        //设置去重
        andPredicate.setDistinct(query.getDistinct());
        orPredicate.setDistinct(query.getDistinct());
        //设置目标实体
        andPredicate.setTargetClass(query.getTargetClass());
        orPredicate.setTargetClass(query.getTargetClass());
